...
public class Minimum_Window_Substring76 {
  ...
  public String minWindow(String s, String t) {
    ...
    while (r < s.length()) {
      ...
      map[currR]--;
      // (Very Important!!!!!!!!)
      // map record all the letters not only the char in the t but also in the s; (Very
      // Important!!!!!!!!)
      // because the letter not in the t will never have a positive frequency, we will never need to
      // worry about it;
      while (counter == 0) {
        if (r - l + 1 < length) {
          length = r - l + 1;
